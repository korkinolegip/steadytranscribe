...
# что может встретиться на пути (cloud — декорация, НЕ препятствие)
_OBSTACLE_KINDS = ["coffee", "file", "phone", "zzz"] + _CUSTOM_KINDS
...
class MiniGame(QWidget):
            ...
            if self._spawn_in <= 0:
                self._obstacles.append(
                    {"x": float(self.width() + 20),
                     ...
                     "kind": random.choice(_OBSTACLE_KINDS)})
    ...
    def paintEvent(self, event):
        ...
        # облака — настоящие (Twemoji), полупрозрачные, лёгкий параллакс
        cloud = self._renderers.get("cloud")
        if cloud is not None:
            p.setOpacity(0.13)
            for c in self._clouds:
                h = c["w"] * 0.62
                cloud.render(p, QRectF(c["x"], c["y"], c["w"], h))
            p.setOpacity(1.0)
